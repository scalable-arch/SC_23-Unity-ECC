...
#define CONSERVATIVE_MODE_START 1 // Rank-level ECC에서 Conservative mode on/off (키면 chip position도 기록해서 다른 chip correction 하면 cacheline 전체를 DUE 처리!)
...
ErrorType ECC::decodeInternal(FaultDomain *fd, CacheLine &errorBlk) {
    ...
    //int Failure_rate=1000; // bit error injection rate
    ...
    */
    ...
    for (int i=errorBlk.getBitN()/codec->getBitN()-1; i>=errorBlk.getBitN()/codec->getBitN()/2; i-=2) { // 31, 29, ... 17
        ...
        if(CONSERVATIVE_MODE_START){
            if(correctedPosSet.size()==1)
                isConservative=isConservative;
            else if(correctedPosSet.size()==2)
                isConservative=1;
            else if(correctedPosSet.size()==3){
                if(correctedPosSet.count(-1)==0) // -1 없는 경우
                    isConservative=1;
            }
            else if(correctedPosSet.size()>=4)
                isConservative=1;
        }
