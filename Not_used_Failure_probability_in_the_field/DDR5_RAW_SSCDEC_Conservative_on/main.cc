...
int main(int argc, char **argv)
{
    ...
    switch (atoi(argv[1])) {
        ...
        case 1:
            dg = new DomainGroupDDR(DIMMcnt/2, 2, 10, 4, 32);
            ecc = new AMDCHIPKILL80b();
            sprintf(filePrefix, "010.4x10.AMDCHIPKILL.%s", argv[3]);
            break;
    ...
    // (DomainGroup *dg, ECC *ecc, Scrubber *scrubber, long runCnt, char* filePrefix, int faultCount, std::string *faults)
    delete tester;
    delete scrubber;
}
