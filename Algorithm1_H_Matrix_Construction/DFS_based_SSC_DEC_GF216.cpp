...
vector<unsigned int> Syndromes; // Keep Syndromes
                                ...
                                // 1,048,560 (65535x16)
unsigned int H_Matrix_binary[SYMBOL_COUNT_RS*COLS_PER_SYMBOL]; // RS-code의 255개 symbol column을 전부 cauchy matrix로 풀었고, 각 column은 GF(2^16) 으로 나타내고 해당 값 저장
...
unsigned int column_index[18]; // 고른 column index (0~254)
...
void DFS(int start, int depth, int last_select_column_index)
{
    ...
    int last_position=65535-depth; // 각 depth마다 선택하는 column의 마지막 위치
    //printf("step3 \n");
