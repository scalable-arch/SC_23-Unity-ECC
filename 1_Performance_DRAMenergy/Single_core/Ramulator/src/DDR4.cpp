...
map<string, enum DDR4::Org> DDR4::org_map = {
    ...
    {"DDR4_16Gb_x4", DDR4::Org::DDR4_16Gb_x4},
};
...
map<string, enum DDR4::Speed> DDR4::speed_map = {
    ...
    {"DDR4_3200", DDR4::Speed::DDR4_3200}, {"DDR4_4800", DDR4::Speed::DDR4_4800},
};
...
void DDR4::init_speed()
{
    ...
    switch (speed_entry.rate) {
        ...
        case 4800: speed = 4; break;
        default: assert(false);
    };
    switch (org_entry.size >> 10){
        std::cout << "org entry size : " << org_entry.size << endl;
        case 2: density = 0; break; // 2Gb
        case 4: density = 1; break; // 4Gb
        case 8: density = 2; break; // 8Gb
        case 16: density = 2; break; // 16Gb!!!!! (newly added!!?)
        default: assert(false);
    }
...
void DDR4::init_timing()
{
    ...
    t[int(Command::WR)].push_back({Command::WR, 1, s.nCCDL_WR});
    t[int(Command::WR)].push_back({Command::WRA, 1, s.nCCDL_WR});
    t[int(Command::WRA)].push_back({Command::WR, 1, s.nCCDL_WR});
    t[int(Command::WRA)].push_back({Command::WRA, 1, s.nCCDL_WR});
