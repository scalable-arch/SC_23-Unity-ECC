...
public:
    ...
    struct OrgEntry {
        ...
        {16<<10,  4, {0, 0, 8, 4, 1<<16, 1<<11}} // chip  width : 4, channel :0, rank : 0, bank group : 8, banks per a bank group : 4 , row : 16bit, column : 11bit
    }, org_entry;
    ...
    int prefetch_size = 16; // 8n prefetch DDR
    int channel_width = 32;

    struct SpeedEntry {
        int rate;
        double freq, tCK;
        int nBL, nCCDS, nCCDL, nCCDL_WR, nRTRS;
        ...
        {3200, 1600, 0.625, prefetch_size/2/*DDR*/,           4,     8,     16,        2,  22,        22,  22, 16,   52, 74, 12,  4,    12,    24, 8,    10,   40, 0,  0,   8,   10,  0,    8,      0,   0},
        //rate, freq, tCK,  nBL,                              nCCDS  nCCDL nCCDL_WR(!) nRTRS nCL(RL!) nRCD nRP nCWL nRAS nRC nRTP nWTRS nWTRL nWR nRRDS nRRDL nFAW nRFC nREFI nPD nXP nXPDLL nCKESR nXS nXSDLL
        {4800, 2400, (1.0/2400)*1000, prefetch_size/2/*DDR*/, 8,     12,     24,        2,  36,        39,  39, 34,     77, 116, 18,  48,     66,        72, 8,   12,    32,    0,  0,   18,   18,  0,    8,      0,   0}
        //rate, freq, tCK,              nBL,                  nCCDS  nCCDL nCCDL_WR(!) nRTRS nCL(RL!) nRCD nRP nCWL(!) nRAS nRC nRTP nWTRS(!) nWTRL(!)  nWR nRRDS nRRDL nFAW nRFC nREFI nPD   nXP nXPDLL nCKESR nXS nXSDLL
    }, speed_entry;
